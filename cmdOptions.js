...
function optionDefinitions() {
  return [
    ...
    { name: 'head-dry-run', type: Number, defaultValue: 10, description: 'print only first N items if --dry-run specified' },
    { name: 'dir-name-format', type: String, defaultValue: 'YYYY-MM', description: 'format of subdirectories name: ' + dateFormats.join(' or ') },
...
function validateCmdOptions() {
  ...
  if (cmdOptions['head'] < 0 || cmdOptions['head-dry-run'] < 0
    ...
    return false
  if (!(dateFormats.includes(cmdOptions['dir-name-format'])))
    return false
  return true
}
